...
- **Release logs directory**: `~/work/skyvalley/source/differ/logs/`

## Logging

**Create a log file at the very start of the release process** to capture all commands and outputs for debugging.

### Log File Setup (Step 0)

```bash
# Create logs directory if it doesn't exist
mkdir -p ~/work/skyvalley/source/differ/logs

# Create timestamped log file
LOG_FILE=~/work/skyvalley/source/differ/logs/release-$(date +%Y%m%d-%H%M%S).log

# Log header
echo "=== Differ Release Log ===" > "$LOG_FILE"
...
For EVERY bash command in the release process, use this pattern to log both the command and its output:

```bash
echo -e "\n[STEP X] Description of step" >> "$LOG_FILE"
echo "$ command_here" >> "$LOG_FILE"
command_here 2>&1 | tee -a "$LOG_FILE"
```

At the end of the process, report the log file path:
```
📋 Full release log: ~/work/skyvalley/source/differ/logs/release-YYYYMMDD-HHMMSS.log
```

**Important**: The logs directory is gitignored - logs are for local debugging only and should not be committed.
